...
export class EventService {
  ...
  // Marketing methods
  static async getEventMarketing(
    id: number
  ): Promise<ApiResponse<EventMarketing>> {
    const response = await fetch(`${API_BASE}/${id}/marketing`);

    if (!response.ok) {
      throw new Error(
        `Failed to fetch event marketing: ${response.statusText}`
      );
    }
  ...
  static async createEventMarketing(
    eventId: number,
    marketingData: CreateEventMarketingRequest
  ): Promise<ApiResponse<EventMarketing>> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to create event marketing: ${response.statusText}`
      );
    }
  ...
  // Volunteer methods
  static async getEventVolunteers(
    id: number
  ): Promise<ApiResponse<EventVolunteer[]>> {
    const response = await fetch(`${API_BASE}/${id}/volunteers`);

    if (!response.ok) {
      throw new Error(
        `Failed to fetch event volunteers: ${response.statusText}`
      );
    }
  ...
  static async addVolunteer(
    eventId: number,
    volunteerData: CreateVolunteerRequest
  ): Promise<ApiResponse<EventVolunteer>> {
  ...
  static async getEventAttendance(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch event attendance: ${response.statusText}`
      );
    }
  ...
  static async checkInAttendee(
    ...
    const response = await fetch(
      `${API_BASE}/attendance/${attendanceId}/checkin`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
