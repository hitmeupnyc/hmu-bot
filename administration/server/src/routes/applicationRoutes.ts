...
router.post('/bulk', effectToExpress((req, res) =>
  Effect.gen(function* () {
    const bodyData = yield* extractBody<{ applications: ApplicationFormData[] }>(req);
    ...
    yield* Effect.forEach(applications, (applicationData, index) =>
      Effect.gen(function* () {
        try {
          // Convert application data to member format
          const memberData = {
          ...
          console.error(`CSV Import - Failed to process application ${index + 1}:`, error);
          results.errors.push({
            index: index + 1,
