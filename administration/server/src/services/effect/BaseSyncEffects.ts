...
import { Member } from './schemas/MemberSchemas';
import {
 ...
 */
export const updateExistingMember = (
  existingMember: Member,
  updates: Partial<Members>
) =>
  Effect.gen(function* () {
    const db = yield* DatabaseService;
    ...
    if (Object.keys(updateData).length > 0) {
      ...
      yield* db.query(async (db) =>
        db
          .updateTable('members')
          .set(updateData)
          .where('id', '=', existingMember.id as unknown as number)
          .execute()
      );
    }
...
export const upsertExternalIntegration = (
  ...
  Effect.gen(function* () {
    ...
    yield* db.query(async (db) =>
      db
        .insertInto('external_integrations')
        .values({
 ...
 */
export const deactivateExternalIntegration = (
  memberId: number,
  systemName: string
) =>
  Effect.gen(function* () {
    ...
    yield* db.query(async (db) =>
      db
        .updateTable('external_integrations')
        .set((eb) => ({
 ...
 */
export const findMemberByExternalId = (
  systemName: string,
  externalId: string
) =>
  Effect.gen(function* () {
...
export const verifyHMACSignature = (verification: HMACVerification) =>
  Effect.gen(function* () {
    const validated = yield* Schema.decodeUnknown(HMACVerificationSchema)(
      verification
    );
 ...
 */
export const verifyMD5Signature = (
  payload: string,
  signature: string,
  secret: string
) =>
  Effect.gen(function* () {
    ...
    const expectedSignature = yield* Effect.try({
      try: () => crypto.createHmac('md5', secret).update(payload).digest('hex'),
      catch: (error) =>
        new HMACVerificationError({
...
export const createBaseMember = (memberData: MemberData) =>
  Effect.gen(function* () {
    const db = yield* DatabaseService;
    const validatedData =
      yield* Schema.decodeUnknown(MemberDataSchema)(memberData);
...
export const createSyncPipeline = <T>(config: {
  ...
  Effect.gen(function* () {
    ...
    const results = yield* Effect.forEach(items, config.processItem, {
  ...
  }).pipe(Effect.catchAll((error) => Effect.succeed({ synced: 0, errors: 1 })));
