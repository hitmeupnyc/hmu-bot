 ...
 */
const updateMemberMembership = (memberId: number, membershipTypeId: number, pledge: PatreonPledge) =>
  Effect.gen(function* () {
    const db = yield* DatabaseService
    
    // End any existing active memberships
    yield* db.query(async (db) =>
      db.updateTable('member_memberships')
    ...
    if (!pledge.attributes.declined_since) {
      yield* db.query(async (db) =>
        db.insertInto('member_memberships')
          .values({
            member_id: memberId,
...
export const syncPledge = (
  ...
  Effect.gen(function* () {
    ...
    const patron = included.find(item => 
      item.type === 'user' && item.id === validatedPledge.relationships.patron.data.id
...
export const exchangePatreonOAuthCode = (code: string, redirectUri: string) =>
  Effect.gen(function* () {
    ...
    return yield* Effect.tryPromise({
      ...
      catch: (error) => new PatreonOAuthError({
...
export const bulkSyncPatreons = (campaignId: string) =>
  Effect.gen(function* () {
    ...
    const pledgesData = yield* Effect.tryPromise({
      ...
      catch: (error) => new PatreonAPIError({
