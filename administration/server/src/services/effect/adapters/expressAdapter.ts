...
import { DatabaseError, ConnectionError, TransactionError } from '../errors/DatabaseErrors';
import { MemberNotFound, EmailAlreadyExists, MemberValidationError } from '../errors/MemberErrors';
...
const toExpressError = (error: unknown): Error => {
  ...
  if (error instanceof DatabaseError || error instanceof ConnectionError || error instanceof TransactionError) {
 ...
 */
export const effectToExpress = <A, E>(
  effectFn: (req: Request, res: Response) => Effect.Effect<A, E, any>
) => async (req: Request, res: Response, next: NextFunction) => {
  try {
    const effect = effectFn(req, res);
    const result = await Effect.runPromiseExit(
      effect.pipe(Effect.provide(DatabaseLive)) as Effect.Effect<A, E, never>
    );
 ...
 */
export const withRequestData = <T>(
 ...
 */
export const extractAuditInfo = (req: Request): Effect.Effect<{ sessionId: string; userIp: string }, Error> =>
  Effect.try({
    try: () => ({
