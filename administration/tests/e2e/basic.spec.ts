...
test.describe('Club Management System', () => {
  test('should load the homepage', async ({ page }) => {
    ...
    await expect(page.getByText('HMU Administration')).toBeVisible();
  ...
  test('should navigate to settings page', async ({ page }) => {
    ...
    await expect(
      page.getByRole('button', { name: 'Configure Eventbrite' })
    ).toBeVisible();
    await expect(
      page.getByRole('button', { name: 'Configure Patreon' })
    ).toBeVisible();
    await expect(
      page.getByRole('button', { name: 'Configure Klaviyo' })
    ).toBeVisible();
  });
});
