...
test.describe('Dashboard Page', () => {
  ...
  test('should display all statistics cards', async ({ page }) => {
    ...
    const totalMembersValue = await page
      .getByTestId('stat-total-members-value')
      .textContent();
    expect(totalMembersValue).not.toBe('--');
    expect(totalMembersValue).toMatch(/^\d+$/);

    // Active members should be a number
    const activeMembersValue = await page
      .getByTestId('stat-active-members-value')
      .textContent();
  ...
  test('should display recent members section', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Recent Members' })
    ).toBeVisible();

    // Should have a table with member information
    const recentMembersSection = page
      .locator('div')
      .filter({ hasText: 'Recent Members' })
      .locator('..');
  ...
  test('should display upcoming events section', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Upcoming Events' }).nth(1)
    ).toBeVisible();

    // Check the second Upcoming Events section (not the stats card)
    const upcomingEventsSection = page
      .locator('.bg-white')
    ...
    const hasNoEvents = await upcomingEventsSection
      .getByText('No events scheduled.')
      .isVisible()
      .catch(() => false);
  ...
  test('should navigate to members page from recent members', async ({
    page,
  }) => {
    ...
    const recentMembersHeading = page.getByRole('heading', {
      name: 'Recent Members',
    });
    ...
    await expect(
      page.getByRole('heading', { level: 1 }).filter({ hasText: preferredName })
    ).toBeVisible();
  ...
  test('should update statistics when data changes', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Add New Member' })
    ).toBeVisible();
    ...
    await page
      .getByLabel('Email *')
      .fill(`dashboard-test-${timestamp}@example.com`);
    ...
    await expect(
      page.getByText(`dashboard-test-${timestamp}@example.com`)
    ).toBeVisible();
