...
test.describe('Member CRUD Operations', () => {
  test('should submit public application form', async ({ page }) => {
    ...
    await page
      ...
      .fill(`jane.smith.${timestamp}@example.com`);
    await page.getByLabel('Birth Year *').fill('1990');

    // Fill out social media URLs (optional)
    await page
      .getByLabel('Primary URL')
      .fill('https://fetlife.com/users/janesmith');

    // Fill out referral information
    await page
      .getByLabel('Referral Source *')
      .selectOption('Friend/Word of mouth');

    // Fill out sponsor information
    await page.getByLabel('Sponsor Name *').fill('Alice Johnson');
    await page
      .getByLabel('Can you ask your sponsor to email us directly? *')
      .check();

    // Fill out experience questionnaires
    await page
      .getByLabel('Describe your experience with kinky/sexy events *')
      .fill(
        'I have attended several community events and am familiar with kink practices.'
      );
    await page
      .getByLabel('Tell us about yourself *')
      .fill(
        'I am a creative professional interested in exploring the community.'
      );
    await page
      .getByLabel('Describe your understanding of consent *')
      .fill(
        'Consent is ongoing, enthusiastic agreement that can be withdrawn at any time.'
      );

    // Optional additional info
    await page
      .getByLabel('Additional Information (Optional)')
      .fill('Looking forward to learning more about the community.');
  ...
  test('should validate required fields in application form', async ({
    page,
  }) => {
    await page.goto('/apply');
    ...
    const nameInvalid = await nameField.evaluate(
    ...
    await expect(
      page.getByRole('heading', { name: 'HMU NYC Application' })
    ).toBeVisible();
  });

  test('should show conditional fields based on referral source', async ({
    page,
  }) => {
    await page.goto('/apply');
    ...
    await expect(
      page.getByLabel('Please explain how you heard about us *')
    ).toBeVisible();
    ...
    await expect(
      page.getByLabel('Which event did you attend? *')
    ).toBeVisible();
  });
  ...
  test('should validate age requirement', async ({ page }) => {
    ...
    await expect(
      page.getByText('You must be 21 or older to apply')
    ).toBeVisible();
  });
  test('should create a new member through the UI', async ({ page }) => {
    ...
    await page
      .getByLabel('Email *')
      .fill(`test.member.${timestamp}@example.com`);
    await page.getByLabel('Pronouns').fill('they/them');
    ...
    await expect(
      page.getByText(`test.member.${timestamp}@example.com`)
    ).toBeVisible();
    const memberRow = page.locator('tr', {
      hasText: `test.member.${timestamp}@example.com`,
    });
