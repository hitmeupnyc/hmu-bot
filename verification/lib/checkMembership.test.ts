...
describe("Membership Checking Logic Tests", () => {
  ...
  describe("Error Scenarios", () => {
    it("throws error when sheet document ID is missing", async () => {
      ...
      await expect(
        checkMembership(contextWithoutSheet, "test@example.com"),
      ).rejects.toThrow("no 'sheet' in KV store");
    });
    ...
    it("throws error when vetted sheet is missing values", async () => {
      ...
      await expect(
        checkMembership(context, "test@example.com"),
      ).rejects.toThrow("Couldn't find the Vetted list.");
    });
    ...
    it("throws error when private sheet is missing values", async () => {
      ...
      await expect(
        checkMembership(context, "test@example.com"),
      ).rejects.toThrow("Couldn't find the Private list.");
    });
    ...
    it("handles invalid sheet ranges", async () => {
      ...
      server.use(
        http.get(
          "https://sheets.googleapis.com/v4/spreadsheets/invalid-range-test/values/*",
          () => {
  ...
  describe("Sheet Fetching Behavior", () => {
    it("fetches both sheets in parallel", async () => {
      ...
      server.use(
        http.get(
          "https://sheets.googleapis.com/v4/spreadsheets/request-tracking/values/*",
          ({ request, params }) => {
    ...
    it("makes requests with correct authentication headers", async () => {
      ...
      server.use(
        http.get(
          "https://sheets.googleapis.com/v4/spreadsheets/auth-test/values/*",
          ({ request }) => {
