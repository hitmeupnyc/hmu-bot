...
describe("Mailjet Email Service", () => {
  ...
  describe("Email Sending", () => {
    ...
    it("includes correct email content and headers", async () => {
      ...
      server.use(
        http.post("https://api.mailjet.com/v3.1/send", async ({ request }) => {
          ...
          return HttpResponse.json({
            Messages: [
              { Status: "success", To: [{ Email: "test@example.com" }] },
            ],
          });
        }),
      );
      ...
      expect(message.TextPart).toBe(
        `Your confirmation code is ${testCode}. It expires in 5 minutes.`,
      );
    });
        ...
        server.use(
          http.post(
            ...
            async ({ request }) => {
              ...
              return HttpResponse.json({
                Messages: [
                  { Status: "success", To: [{ Email: capturedEmail }] },
                ],
              });
            },
          ),
  ...
  describe("OTP Code Variations", () => {
    it("handles different code formats correctly", async () => {
      ...
      for (const code of testCodes) {
        ...
        expect(capturedMessage.TextPart).toBe(
  ...
  describe("Response Handling", () => {
    ...
    it("handles partial failure responses", async () => {
      ...
      expect(responseData.Messages[0].Errors[0].ErrorCode).toBe(
        "invalid-email",
      );
    });
  });
  ...
  describe("Network Error Handling", () => {
    it("handles network timeouts and connection errors", async () => {
      ...
      await expect(
        sendEmail("test@example.com", testCode, testAuth),
      ).rejects.toThrow();
    });
  ...
  describe("Email Template Validation", () => {
    it("maintains consistent email template structure", async () => {
      ...
      expect(capturedMessage.TextPart).toMatch(
        /^Your confirmation code is \d{6}\. It expires in 5 minutes\.$/,
      );
    });
