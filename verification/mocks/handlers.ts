  ...
  http.get(
    ...
    ({ params, request }) => {
      ...
      switch (sheetId) {
        ...
        case "private-missing-values":
          const decodedRangePrivateMissing = decodeURIComponent(
            range as string,
          );
          if (decodedRangePrivateMissing.includes("Vetted Members")) {
            return HttpResponse.json({
              values: [["test@example.com"]],
            });
          } else if (decodedRangePrivateMissing.includes("Private Members")) {
            return HttpResponse.json({
              range: decodedRangePrivateMissing,
              majorDimension: "ROWS",
        ...
        case "case-sensitivity":
          const decodedRangeCaseSensitivity = decodeURIComponent(
            range as string,
          );
          if (decodedRangeCaseSensitivity.includes("Vetted Members")) {
            return HttpResponse.json({
              values: [["test@example.com"]], // Lowercase in sheet
            });
          } else if (decodedRangeCaseSensitivity.includes("Private Members")) {
            return HttpResponse.json({
              values: [], // Empty private list
            });
          }
          return HttpResponse.json({ values: [] });

        case "complex-malformed":
          const decodedRangeComplex = decodeURIComponent(range as string);
          if (decodedRangeComplex.includes("Vetted Members")) {
            return HttpResponse.json({
              ...defaultResponse,
              values: [
                [], // Empty row
                ...
                [null, "valid@example.com", undefined], // Mixed with nulls
              ],
            });
          }
          return HttpResponse.json({
